...
const platformContactDetails = [
  ...
  { icon: Phone, label: 'Phone', value: '+63 935 254 1057', href: 'tel:+639352541057', color: '#1fa971' },
  { icon: MapPin, label: 'Location', value: 'Baguio City, Philippines', href: undefined, color: '#f5a524' },
]

const accentStyle = (color: string) => ({ '--accent': color }) as CSSProperties

type SchoolContact = { companyName: string; email: string; phone: string | null; address: string | null }
type FreelanceContact = { teacherName: string; email: string; phone: string | null }

function DashboardSidebar({ role, open, onNavigate, collapsed, onToggleCollapse }: DashboardSidebarProps) {
  ...
  useEffect(() => {
    ...
    async function resolve() {
      const sourceTable = role === ROLE.TEACHER ? 'company_organizational_chart' : 'student_lists'

      const { data } = await supabase.from(sourceTable).select('company_code').eq('email', myEmail as string).limit(1)
      ...
      if (companyCode) {
        ...
        if (row) {
          setSchoolContact({ companyName: row.company_name, email: row.email, phone: row.phone_number, address: row.address })
        }
        return
      }
      ...
      if (teacherRow) {
        setFreelanceContact({ teacherName: teacherRow.names, email: teacherRow.email, phone: teacherRow.phone })
      }
    }
  ...
  const contactDetails =
    isEmployee && schoolContact
      ? [
          { icon: Mail, label: 'Email', value: schoolContact.email, href: `mailto:${schoolContact.email}`, color: '#2f6bff' },
          schoolContact.phone && {
            icon: Phone,
            label: 'Phone',
      ...
      : isEmployee && freelanceContact
        ? [
            { icon: Mail, label: 'Email', value: freelanceContact.email, href: `mailto:${freelanceContact.email}`, color: '#2f6bff' },
  ...
  return (
    <>
      {open && <div className="dash-sidebar-overlay" onClick={onNavigate} aria-hidden="true" />}
      <aside className={`dash-sidebar ${open ? 'is-open' : ''} ${collapsed ? 'is-collapsed' : ''}`}>
        <button
          className="dash-sidebar-collapse-btn"
          onClick={onToggleCollapse}
          aria-label={collapsed ? 'Expand sidebar' : 'Collapse sidebar'}
        >
          <ChevronLeft size={14} className={collapsed ? 'is-flipped' : ''} />
        </button>
        ...
        <div className="dash-sidebar-scroll">
          ...
          <div className="dash-support-card">
            ...
            <button className="dash-support-btn" onClick={() => setShowContact(true)}>
              Contact Support
            </button>
          </div>
        </div>
      ...
      {showContact && (
        <div className="dash-support-overlay" onClick={() => setShowContact(false)}>
          <div className="dash-support-modal" onClick={(e) => e.stopPropagation()}>
            <div className="dash-support-modal-banner">
              <button className="dash-support-modal-close" aria-label="Close" onClick={() => setShowContact(false)}>
                <X size={18} />
              </button>
              ...
              {contactDetails.map((item) => {
                const rowContent = (
                  <>
                    ...
                    <div>
                      <span className="dash-support-row-label">{item.label}</span>
                      <span className="dash-support-row-value">{item.value}</span>
                    </div>
                  </>
                )
                return item.href ? (
                  <a key={item.label} className="dash-support-row" href={item.href} style={accentStyle(item.color)}>
                    {rowContent}
                  </a>
                ) : (
                  <div key={item.label} className="dash-support-row" style={accentStyle(item.color)}>
                    {rowContent}
                  </div>
                )
              })}
            ...
            <p className="dash-support-modal-note">We typically respond within 24 hours.</p>
          </div>
        </div>
      )}
    </>
