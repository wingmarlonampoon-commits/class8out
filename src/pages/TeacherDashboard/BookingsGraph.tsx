...
function BookingsGraph() {
  ...
  return (
    <div className="teacher-dash-graph-panel">
      <div className="teacher-dash-graph-header">
        <h2>Bookings Overview</h2>
        <button className="teacher-dash-graph-range" type="button">
          This Week <ChevronDown size={14} />
        </button>
      </div>

      <div className="teacher-dash-graph-chart-wrap">
        <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="teacher-dash-graph-svg" preserveAspectRatio="none">
          <defs>
            <linearGradient id="teacherDashGraphFill" x1="0" y1="0" x2="0" y2="1">
          ...
          {yTicks.map((tick) => {
            ...
            return (
              <g key={tick}>
                <line x1={PADDING.left} x2={WIDTH - PADDING.right} y1={y} y2={y} className="teacher-dash-graph-grid-line" />
                <text x={PADDING.left - 10} y={y + 4} className="teacher-dash-graph-tick-label" textAnchor="end">
                  {tick}
                </text>
              </g>
            )
          ...
          <path d={areaPath} fill="url(#teacherDashGraphFill)" stroke="none" />
          <path d={linePath} className="teacher-dash-graph-line" fill="none" />

          {points.map((p, i) => (
            <g key={i}>
              {hoverIndex === i && (
                <line x1={p.x} x2={p.x} y1={PADDING.top} y2={baseline} className="teacher-dash-graph-hover-guide" />
              )}
              <circle cx={p.x} cy={p.y} r={hoverIndex === i ? 5 : 3.5} className="teacher-dash-graph-point" />
              <circle
                cx={p.x}
                cy={p.y}
                r={16}
              ...
              />
              <text x={p.x} y={HEIGHT - 22} className="teacher-dash-graph-x-label" textAnchor="middle">
                {weekDates[i].label}
              </text>
              <text x={p.x} y={HEIGHT - 8} className="teacher-dash-graph-x-sublabel" textAnchor="middle">
        ...
        {active && hoverIndex !== null && (
          <div
            className="teacher-dash-graph-tooltip"
      ...
      <div className="teacher-dash-graph-stats-strip">
        <div className="teacher-dash-graph-stat">
          <span className="teacher-dash-graph-stat-label">Total</span>
          <span className="teacher-dash-graph-stat-value">{total}</span>
        </div>
        <span className="teacher-dash-graph-stats-divider" />
        <div className="teacher-dash-graph-stat">
          <span className="teacher-dash-graph-stat-label">Avg / day</span>
          <span className="teacher-dash-graph-stat-value">{average}</span>
        </div>
        <span className="teacher-dash-graph-stats-divider" />
        <div className="teacher-dash-graph-stat">
          <span className="teacher-dash-graph-stat-label">Peak day</span>
          <span className="teacher-dash-graph-stat-value">{total === 0 ? '—' : weekDates[peakIndex].label}</span>
        </div>
      </div>
    </div>
  )
