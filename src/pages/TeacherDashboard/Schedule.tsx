...
const RPC_ERROR_MESSAGES: Record<string, string> = {
  ...
  NOT_AUTHORIZED:
    'Not authorized. Your school may have turned off self-booking.',
  STUDENT_NOT_FOUND: 'Could not find that student.',
  STUDENT_WRONG_COMPANY:
    'That student belongs to a different company.',
  STUDENT_NOT_YOURS: "That student isn't on your roster.",
  INSUFFICIENT_CREDITS:
    'This student has no class credits left.',
  SLOT_ALREADY_BOOKED:
    'This slot was just booked by someone else.',
  CLASS_NOT_STARTED:
    "You can mark this completed once the class's scheduled time has passed.",
}
...
function StudentDetailLines({
  ...
  return (
    <div className="teacher-schedule-student-detail">
        ...
        <p key={i}>
          ...
          {/^https?:\/\//i.test(c.handle) ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="teacher-schedule-contact-link"
            >
      ...
      {student.description && (
        <p className="teacher-schedule-student-detail-desc">
          {student.description}
        </p>
      )}
    </div>
...
function Schedule() {
  ...
  const [selfProfile, setSelfProfile] =
    useState<SelfProfile | null>(null)
  ...
  const [weekCells, setWeekCells] =
    useState<Map<string, ClassCell>>(
      new Map(),
    )
  ...
  const [cellsReloadToken, setCellsReloadToken] =
    useState(0)
  ...
  const [cellStudentSearch, setCellStudentSearch] =
    useState('')

  const [cellActionLoading, setCellActionLoading] =
    useState(false)

  const [cellMessage, setCellMessage] =
    useState<Message | null>(null)

  const [expandedStudentId, setExpandedStudentId] =
    useState<string | null>(null)

  const [cellSubject, setCellSubject] =
    useState('')

  const [cellBookId, setCellBookId] =
    useState('')

  const [cellNotesDraft, setCellNotesDraft] =
    useState('')

  const [cellNotesSaving, setCellNotesSaving] =
    useState(false)
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .select(
        'photo, intro_video, intro_message, contact, subjects, "Rating", phone',
      )
      .eq('id', identity.teacherId)
      .single()
      .then(({ data }) => {
        setSelfProfile(
          (data as SelfProfile) ?? null,
        )
      })
  }, [identity])
    ...
    const query =
      identity.kind === 'freelance'
        ? supabase
            ...
            .select(
              'id, name, email, student_code, "Credits", subject, english_level, description, contact, books',
            )
            .eq(
              'teacher_id',
              identity.teacherId,
  ...
  useEffect(() => {
    ...
    if (identity.kind === 'freelance') {
      Promise.all([
        ...
        setAvailableBooks([
          ...((own.data as BookOption[]) ?? []),
          ...((pub.data as BookOption[]) ?? []),
        ])
      })
    ...
    supabase
      ...
      .or(
        `company_code.eq.${identity.companyCode},PublicAvailability.eq.true`,
      )
      .order('subject', {
        ascending: true,
      })
  ...
  useEffect(() => {
    ...
    supabase
      .from(classesTable)
      .select(
        'id, teacher_id, student_id, date, start_time, "Status", class_details, class_notes',
      )
      .eq(
        'teacher_id',
        identity.teacherId,
      ...
      .then(({ data }) => {
        const map =
          new Map<string, ClassCell>()

        ;((data as ClassCell[]) ?? []).forEach(
          (row) => {
            map.set(
              `${row.date}_${row.start_time}`,
              row,
            )
          },
        )
  ...
  // Drives the "now" line: re-evaluate the current
  // time once a minute so the line advances without
  ...
  useEffect(() => {
    ...
    const row = weekCells.get(
      `${selectedCell.date}_${selectedCell.startTime}`,
    )
  ...
  const teacherTimezone = identity
    ? identity.kind === 'company'
      ? identity.companySettings.timezone
      : identity.settings.timezone
    : 'Asia/Manila'

  void nowTick

  const today =
    getZonedNow(teacherTimezone)
  ...
  const canBookOrCancel = identity
    ? identity.kind === 'freelance' ||
      identity.selfBookingAllowed
    : false
    ...
    if (identity.kind === 'freelance') {
      ...
      if (error) {
        setCellMessage({
          type: 'error',
          text:
            'Could not open this slot. Please try again.',
        })
        return
      }
  ...
  const handleMarkClosed = async (
    ...
    if (
      ...
      setCellMessage({
        type: 'error',
        text:
          'Could not close this slot. It may have just been booked.',
      })
      return
    }
    ...
    const classDetails:
      ...
      selectedBook
        ? {
            ...
            book_label:
              selectedBook
                ? `${selectedBook.subject} — ${selectedBook.category}`
                : null,
          }
        : null
    ...
    if (error) {
      setCellMessage({
        ...
        text:
          ...
          'Could not book this student. Please try again.',
      })
      return
    }
  ...
  const handleMarkCompleted = async (
    ...
    const { error } =
      identity?.kind === 'freelance'
        ? await supabase.rpc(
            'mark_freelance_class_completed',
            {
              p_class_id: rowId,
            },
          )
        : await supabase.rpc(
            'mark_class_completed',
            {
              p_class_id: rowId,
            },
          )
    ...
    if (error) {
      setCellMessage({
        ...
        text:
          ...
          'Could not mark this class completed. Please try again.',
      })
      return
    }
  ...
  return (
    <div className="teacher-schedule-page">
      ...
      {identityLoading ||
      !identity ? (
        <p className="teacher-schedule-loading">
          Loading…
        </p>
      ) : (
        <>
              ...
              <div className="teacher-schedule-detail-panel">
                ...
                <div className="teacher-schedule-detail-body">
                  <div className="teacher-schedule-detail-col">
                    {selfProfile.subjects &&
                      selfProfile.subjects.length >
                        0 && (
                        <div className="teacher-schedule-detail-block">
                          <span className="teacher-schedule-field-label">
                            Subjects
                          </span>

                          <div className="teacher-schedule-chip-row">
                            {selfProfile.subjects.map(
                              (s) => (
                                <span
                                  key={s}
                                  className="teacher-schedule-chip"
                                >
                                  {s}
                                </span>
                              ),
                    ...
                    <div className="teacher-schedule-detail-block">
                      ...
                      <p className="teacher-schedule-contact-line">
                        {selfProfile.phone}
                      </p>

                      {selfProfile.contact?.contacts?.map(
                        (c, i) => (
                          <p
                            key={i}
                            className="teacher-schedule-contact-line"
                          >
                            <strong>
                              {c.platform}:
                            </strong>{' '}
                    ...
                    {selfProfile.intro_message && (
                      <div className="teacher-schedule-detail-block">
                        ...
                        <p className="teacher-schedule-intro-message">
                          {
                            selfProfile.intro_message
                          }
                        </p>
          ...
          {identity.kind ===
            ...
            !identity.selfBookingAllowed && (
              <p className="teacher-schedule-field-help">
                Your school has turned off
                self-booking. You can view
                your schedule, but booking
                changes must go through your
                admin.
              </p>
            )}
          ...
          <div className="teacher-schedule-week-panel">
            <div className="teacher-schedule-week-nav">
              <button
                type="button"
                className="teacher-schedule-week-nav-btn"
                onClick={prevWeek}
              ...
              <div className="teacher-schedule-week-nav-center">
                <span className="teacher-schedule-week-label">
                  {weekLabel}
                </span>

                <button
                  type="button"
                  className="teacher-schedule-today-btn"
                  onClick={
                    goToToday
                  }
                >
            ...
            <div className="teacher-schedule-grid-wrap">
              {weekLoading && (
                <p className="teacher-schedule-grid-loading">
                  Loading…
                </p>
              )}

              <div
                className="teacher-schedule-grid"
                style={{
                  gridTemplateColumns:
              ...
              >
                ...
                {weekDays.map((d) => (
                  <div
                    key={d.toISOString()}
                    className={`teacher-schedule-grid-day-header ${
                      isSameDay(
                        d,
                        today,
                      )
                  ...
                  >
                    <span className="teacher-schedule-grid-day-name">
                    ...
                    <span className="teacher-schedule-grid-day-num">
                      {d.getDate()}
                    </span>
                  </div>
                ))}
                ...
                {timeSlots.map(
                  (
                    ...
                    >
                      <div className="teacher-schedule-grid-time-label">
                        {slot.label}
                      </div>
                        ...
                        (
                          ...
                          return (
                            ...
                            >
                              ...
                              {showNowLine && (
                                <span
                                  className="teacher-schedule-now-line"
                                  style={{
                                    top: `${
                                      nowFraction *
                                      100
      ...
      {selectedCell &&
        selectedCellInfo && (
          <div
            className="teacher-schedule-modal-overlay"
            onClick={
              closeCellPopup
            }
          >
            <div
              className="teacher-schedule-cell-modal"
              onClick={(e) =>
                e.stopPropagation()
              }
            >
              <button
                className="teacher-schedule-modal-close"
                aria-label="Close"
                onClick={
                  closeCellPopup
                }
              ...
              <div className="teacher-schedule-cell-modal-header">
                ...
                <p className="teacher-schedule-cell-modal-time">
                  {formatTimeLabel(
                    selectedCell.startTime,
                  )}
                </p>
                            ...
                            (s) => {
                              ...
                              return (
                                <div
                                  ...
                                  className="teacher-schedule-student-item"
                                >
                                  <button
                                    type="button"
                                    className="teacher-schedule-student-row"
              ...
              {cellMessage && (
                <p
                  className={`teacher-schedule-message is-${cellMessage.type}`}
                >
                  {
                    cellMessage.text
                  }
              ...
              <div className="teacher-schedule-modal-footer">
                ...
                <span>
                  Booking spends 1 class
                  credit; canceling refunds
                  it.
                </span>
              </div>
            </div>
